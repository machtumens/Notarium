...
const VoiceRecorder: React.FC<VoiceRecorderProps> = ({
  ...
  React.useEffect(() => {
    if (isRecording) {
      ...
      // eslint-disable-next-line react-hooks/set-state-in-effect -- resets timer when recording stops; behavior-preserving
      setTime(0);
    }
    return () => {
      ...
      <div className="w-full h-10 flex items-center justify-center gap-0.5 px-4">
        {[...Array(visualizerBars)].map((_, i) => (
          <div
            ...
            style={{
              // eslint-disable-next-line react-hooks/purity -- decorative audio-visualizer bar heights are intentionally randomized per render
              height: `${Math.max(15, Math.random() * 100)}%`,
              animationDelay: `${i * 0.05}s`,
              // eslint-disable-next-line react-hooks/purity -- decorative audio-visualizer animation timing is intentionally randomized per render
...
export const PromptInputBox = React.forwardRef(
  (props: PromptInputBoxProps, ref: React.Ref<HTMLDivElement>) => {
    ...
    const processFile = (file: File) => {
      if (!isImageFile(file)) {
        logger.debug('ai-prompt', 'Non-image file rejected');
        return;
      }
      if (file.size > 10 * 1024 * 1024) {
        logger.debug('ai-prompt', 'File too large (max 10MB)');
        return;
      }
      setFiles([file]);
